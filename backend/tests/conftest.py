...
other or ever touch the real cirqle.db. The session fixture patches
`app.db.engine` directly (not just the FastAPI `get_session` dependency) so
that background tasks and any other code that opens its own `Session(engine)`
also land on the test database, not the real one.
"""
import os
import sys
